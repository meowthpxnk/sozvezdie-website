...
const COMPOSER_SPLIT_BP = 581;
const IMAGES_COLUMN_WIDTH = 400;
...
const ComposerLayout = styled.div`
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 20px;

    @media (min-width: ${COMPOSER_SPLIT_BP}px) {
        flex-direction: row;
...
const ImagesColumn = styled.div`
    width: 100%;
    max-width: ${IMAGES_COLUMN_WIDTH}px;
    flex-shrink: 0;
    box-sizing: border-box;

    @media (min-width: ${COMPOSER_SPLIT_BP}px) {
        width: min(${IMAGES_COLUMN_WIDTH}px, 42%);
        max-width: ${IMAGES_COLUMN_WIDTH}px;
        margin: 0;
    }
`;
...
const CheckboxBox = styled.span<{ $checked: boolean }>`
    ...
    background: ${(props) =>
        props.$checked ? "var(--main-color)" : "transparent"};
`;
...
export type ComposerProductImage = {
    id: string;
    url: string;
    file?: File;
    existingUuid?: string;
...
export type AuthorProductForm = {
    ...
    fandomSlug: string;
    fandomLabel: string;
    images: ComposerProductImage[];
    coverImageId: string;
    isAdult: boolean;
};

type AuthorProductComposerProps = {
    initialForm?: AuthorProductForm;
    shellTitle?: string;
    formTitle?: string;
...
export const AuthorProductComposer = ({
    initialForm,
    ...
    const [form, setForm] = useState<AuthorProductForm>({
        name: "",
        description: "",
        price: "",
        stockCount: "1",
    ...
    const hydratedInitialFormKeyRef = useRef(false);

    useEffect(() => {
        if (!initialForm || hydratedInitialFormKeyRef.current) {
            return;
        }

        hydratedInitialFormKeyRef.current = true;
        setForm(initialForm);
    }, [initialForm]);
    ...
    const handleSubcategoryChange = useCallback((label: string, slug: string) => {
        setForm((prev) => {
            if (prev.subcategoryLabel === label && prev.subcategorySlug === slug) {
                return prev;
            }
    ...
    const handleFandomChange = useCallback((label: string, slug: string) => {
        setForm((prev) => {
            if (prev.fandomLabel === label && prev.fandomSlug === slug) {
                return prev;
            }
    ...
    const validationError = useMemo(() => {
        ...
        if (!Number.isFinite(Number(form.stockCount)) || Number(form.stockCount) < 1) {
            return "Количество должно быть не меньше 1";
        }
        if (form.images.length === 0) {
            return "Добавьте хотя бы одно изображение";
        }
        if (mode === "create" && form.images.some((image) => !image.file)) {
            return "Загрузите изображения товара";
        }
        if (mode === "edit" && form.images.some((image) => !image.file && !image.existingUuid)) {
            return "Некорректный список изображений";
        }
        return "";
    }, [form, mode]);
    ...
    useEffect(() => {
        ...
        onRegisterSubmit(async () => {
            ...
            setError("");
            await onSubmitForm(currentForm);
        });
    }, [onRegisterSubmit, onSubmitForm, validationError]);
    ...
    if (loading) {
        return (
            <>
                ...
                <LoadingMessage>Загрузка товара…</LoadingMessage>
            </>
        );
    }
                    ...
                    <ComposerLayout>
                                ...
                                <ImagesRailCard>
                                    ...
                                    <ProductImagesRow
                                        ...
                                        onReorder={(nextImages) => {
                                            const typedNextImages = nextImages as ComposerProductImage[];
                        ...
                        <FieldsColumn>
                            ...
                            <FieldGroup>
                                ...
                                >
                                    <CheckboxBox $checked={Boolean(form.isAdult)} />
                                    <CheckboxCopy>
                                        Товар 18+
                                        <CheckboxHint>
                                            Фотографии будут размыты, пока покупатель не
                                            подтвердит возраст
                                        </CheckboxHint>
                                    </CheckboxCopy>
                                </CheckboxRow>
                            ...
                            <FieldGroup>
                                <FieldLabel htmlFor="product-stock">
                                    Количество на складе <RequiredMark>*</RequiredMark>
                                </FieldLabel>
                                <NumberInput
                                    id="product-stock"
                                    type="number"
                            ...
                            {(error || validationError) ? (
                                <ErrorText>{error || validationError}</ErrorText>
                            ) : null}
                            {hideSubmit ? null : (
                        ...
                        <ModalActions>
                            ...
                            <ModalConfirmButton
                                ...
                                onClick={() => {
                                    ...
                                    setSaving(true);
                                    const submitPromise = onSubmitForm(formRef.current);
                                    void submitPromise
                                        .then(() => {
                                            setConfirmModalOpen(false);
